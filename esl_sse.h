 ...
 *****************************************************************/
extern simde__m128  esl_sse_logf(simde__m128 x);
extern simde__m128  esl_sse_expf(simde__m128 x);
extern void    esl_sse_dump_ps(FILE *fp, simde__m128 v);
...
esl_sse_hmax_epu8(simde__m128i a)
{
  a = simde_mm_max_epu8(a, simde_mm_srli_si128(a, 8));
  a = simde_mm_max_epu8(a, simde_mm_srli_si128(a, 4));
  a = simde_mm_max_epu8(a, simde_mm_srli_si128(a, 2));
  a = simde_mm_max_epu8(a, simde_mm_srli_si128(a, 1));
  return (uint8_t) simde_mm_extract_epi16(a, 0);   /* only low-order 8 bits set; so _epi16 or _epi8 equiv; _epi8 is SSE4.1 */
}
...
esl_sse_hmax_epi8(simde__m128i a)
{
  a  = simde_mm_max_epi8(a, simde_mm_shuffle_epi32  (a, SIMDE_MM_SHUFFLE(2,3,0,1)));  //  SIMDE_MM_SHUFFLE() args are reversed.SIMDE_MM_SHUFFLE(3,2,1,0) is a no-op, for example.
  a  = simde_mm_max_epi8(a, simde_mm_shuffle_epi32  (a, SIMDE_MM_SHUFFLE(0,1,2,3)));
  a  = simde_mm_max_epi8(a, simde_mm_shufflelo_epi16(a, SIMDE_MM_SHUFFLE(2,3,0,1)));
  a  = simde_mm_max_epi8(a, simde_mm_srli_epi16     (a, 8));
  return (int8_t) simde_mm_cvtsi128_si32(a);
}
#endif
...
esl_sse_hmax_epi16(simde__m128i a)
{
  a  = simde_mm_max_epi16(a, simde_mm_shuffle_epi32  (a, SIMDE_MM_SHUFFLE(1,0,3,2)));  
  a  = simde_mm_max_epi16(a, simde_mm_shufflelo_epi16(a, SIMDE_MM_SHUFFLE(1,0,3,2)));
  a  = simde_mm_max_epi16(a, simde_mm_srli_epi32(a, 16));
  return (int16_t) simde_mm_cvtsi128_si32(a);
}
 ...
 */
static inline void
esl_sse_hmax_ps(simde__m128 a, float *ret_max)
{
  a = simde_mm_max_ps(a, simde_mm_shuffle_ps(a, a, SIMDE_MM_SHUFFLE(0, 3, 2, 1)));
  a = simde_mm_max_ps(a, simde_mm_shuffle_ps(a, a, SIMDE_MM_SHUFFLE(1, 0, 3, 2)));
  simde_mm_store_ss(ret_max, a);
}
 ...
 */
static inline void
esl_sse_hmin_ps(simde__m128 a, float *ret_min)
{
  a = simde_mm_min_ps(a, simde_mm_shuffle_ps(a, a, SIMDE_MM_SHUFFLE(0, 3, 2, 1)));
  a = simde_mm_min_ps(a, simde_mm_shuffle_ps(a, a, SIMDE_MM_SHUFFLE(1, 0, 3, 2)));
  simde_mm_store_ss(ret_min, a);
}
 ...
 */
static inline void
esl_sse_hsum_ps(simde__m128 a, float *ret_sum)
{
  a = simde_mm_add_ps(a, simde_mm_shuffle_ps(a, a, SIMDE_MM_SHUFFLE(0, 3, 2, 1)));
  a = simde_mm_add_ps(a, simde_mm_shuffle_ps(a, a, SIMDE_MM_SHUFFLE(1, 0, 3, 2)));
  simde_mm_store_ss(ret_sum, a);
}
...
esl_sse_rightshift_int8(simde__m128i a, simde__m128i neginfmask)
{
  return simde_mm_or_si128(simde_mm_slli_si128(a, 1), neginfmask);
}
 ...
 */
static inline simde__m128i
esl_sse_rightshift_int16(simde__m128i a, simde__m128i neginfmask)
{
  return simde_mm_or_si128( simde_mm_slli_si128(a, 2), neginfmask);
}
...
esl_sse_rightshiftz_float(simde__m128 a)
{
  // Tricky. IEEE754 representation of zero is all 0 bits, so shift alone suffices.
  return (simde__m128) simde_mm_slli_si128( (simde__m128i) a, 4);
}
 ...
 */
static inline simde__m128
esl_sse_leftshiftz_float(simde__m128 a)
{
  // Same trick. IEEE754 representation of zero is all 0 bits.
  return (simde__m128) simde_mm_srli_si128( (simde__m128i) a, 4);
}
 ...
 */
static inline simde__m128 
esl_sse_rightshift_ps(simde__m128 a, simde__m128 b)
{
  return simde_mm_move_ss(simde_mm_shuffle_ps(a, a, SIMDE_MM_SHUFFLE(2, 1, 0, 0)), b);
}
 ...
 */
static inline simde__m128
esl_sse_leftshift_ps(simde__m128 a, simde__m128 b)
{
  register simde__m128 v = simde_mm_move_ss(a, b);                 /* now b[0] a[1] a[2] a[3] */
  return simde_mm_shuffle_ps(v, v, SIMDE_MM_SHUFFLE(0, 3, 2, 1));  /* now a[1] a[2] a[3] b[0] */
}
 ...
 */
static inline int 
esl_sse_any_gt_epu8(simde__m128i a, simde__m128i b)
{
  simde__m128i mask    = simde_mm_cmpeq_epi8(simde_mm_max_epu8(a,b), b); /* anywhere a>b, mask[z] = 0x0; elsewhere 0xff */
  int   maskbits  = simde_mm_movemask_epi8(simde_mm_xor_si128(mask,  simde_mm_cmpeq_epi8(mask, mask))); /* the xor incantation is a bitwise inversion */
  return maskbits != 0;
}
 ...
 */
static inline int 
esl_sse_any_gt_epi16(simde__m128i a, simde__m128i b)
{
  return (simde_mm_movemask_epi8(simde_mm_cmpgt_epi16(a,b)) != 0); 
}
 ...
 */
static inline int 
esl_sse_any_gt_ps(simde__m128 a, simde__m128 b)
{
  simde__m128 mask    = simde_mm_cmpgt_ps(a,b);
  int   maskbits = simde_mm_movemask_ps( mask );
  return maskbits != 0;
}
 ...
 */
static inline simde__m128
esl_sse_select_ps(simde__m128 a, simde__m128 b, simde__m128 mask)
{
  b = simde_mm_and_ps(b, mask);
  a = simde_mm_andnot_ps(mask, a);
  return simde_mm_or_ps(a,b);
}
